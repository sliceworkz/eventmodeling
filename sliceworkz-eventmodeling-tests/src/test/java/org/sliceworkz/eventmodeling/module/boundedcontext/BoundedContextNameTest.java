...
/**
 * A bounded context answers to its own name.
 * <p>
 * Plain {@code @Test}: this is about how the context is wired, which no storage can change.
 */
public class BoundedContextNameTest extends AbstractMockDomainTest {
	...
	void aBoundedContextReportsTheNameItWasBuiltWith ( ) {
		Mock domain = buildBoundedContext(
				BoundedContext.newBuilder(Mock.class)
					.name("UnitTestBoundedContext")
					.eventStorage(eventStorage())
					.instance(InstanceFactory.determine("unittests")));

		assertEquals("UnitTestBoundedContext", domain.name());
	}
